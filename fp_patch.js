...
/* 9 — SYNC DRIVE COURRIERS IT — fichier séparé GED_FO_IT_data.json */
(function(){

var IT_DRIVE_FILE_KEY="fo_it_fid";
var IT_DRIVE_FILENAME="GED_FO_IT_data.json";
var itDriveFileId=localStorage.getItem(IT_DRIVE_FILE_KEY)||null;
var itSyncing=false;

/* ── Lire le token Drive depuis la GED ── */
function getToken(){
  return window.accessToken||null;
}
function isConnected(){
  return !!(window.driveConnected&&getToken());
}

/* ── Trouver ou créer le fichier IT sur Drive ── */
async function itGetOrCreateFile(){
  var tok=getToken();
  if(!tok)return null;
  if(itDriveFileId)return itDriveFileId;
  /* Chercher */
  try{
    var r=await fetch("https://www.googleapis.com/drive/v3/files?q=name%3D'"+IT_DRIVE_FILENAME+"'%20and%20trashed%3Dfalse&fields=files(id,name)",{
      headers:{Authorization:"Bearer "+tok}
    });
    var d=await r.json();
    if(d.files&&d.files.length>0){
      itDriveFileId=d.files[0].id;
      localStorage.setItem(IT_DRIVE_FILE_KEY,itDriveFileId);
      return itDriveFileId;
    }
  }catch(e){}
  /* Créer */
  try{
    var meta={name:IT_DRIVE_FILENAME,description:"Courriers Inspection du Travail FO O-I Wingles"};
    var payload=JSON.stringify({itCourriers:[],lastSaved:new Date().toISOString()});
  ...
  }catch(e){console.warn("[IT Drive] create error:",e);}
  return null;
}

/* ── Sauvegarder courriers sur Drive ── */
async function itSaveToDrive(){
  if(itSyncing)return;
  if(!isConnected())return;
  itSyncing=true;
  try{
    var fid=await itGetOrCreateFile();
    if(!fid){itSyncing=false;return;}
    var payload=JSON.stringify({itCourriers:itData,lastSaved:new Date().toISOString()});
    await fetch("https://www.googleapis.com/upload/drive/v3/files/"+fid+"?uploadType=media",{
      method:"PATCH",
      headers:{"Content-Type":"application/json","Authorization":"Bearer "+getToken()},
      body:payload
    });
    console.info("[IT Drive] Sauvegardé:",itData.length,"courriers");
  }catch(e){
    console.warn("[IT Drive] save error:",e);
  }
  itSyncing=false;
}

/* ── Charger courriers depuis Drive ── */
async function itLoadFromDrive(){
  ...
  try{
    ...
    if(d.itCourriers&&Array.isArray(d.itCourriers)){
      /* Fusionner : garder le plus récent par ID */
      var local=JSON.parse(localStorage.getItem("fo_it")||"[]");
      var merged={};
      local.forEach(function(c){merged[c.id]=c;});
      d.itCourriers.forEach(function(c){merged[c.id]=c;});
      itData=Object.values(merged);
      localStorage.setItem("fo_it",JSON.stringify(itData));
      if(document.getElementById("it-list"))itRender();
      console.info("[IT Drive] Chargé:",itData.length,"courriers");
      return true;
    }
  }catch(e){
    console.warn("[IT Drive] load error:",e);
  }
  return false;
}

/* ── Patcher itSave pour déclencher la sync Drive ── */
var _origItSave=window.itSave;
window.itSave=function(){
  _origItSave();
  if(isConnected())itSaveToDrive();
};

var _origItDel=window.itDel;
window.itDel=function(id){
  _origItDel(id);
  if(isConnected())itSaveToDrive();
};

/* ── Charger au démarrage si Drive déjà connecté ── */
function tryLoad(){
  if(isConnected()){
    itLoadFromDrive();
  }
...
/* ── Surveiller la connexion Drive ── */
var checkCount=0;
var checkTimer=setInterval(function(){
  checkCount++;
  if(checkCount>120){clearInterval(checkTimer);return;}
  if(isConnected()){
...
/* ── Exposer pour appel manuel ── */
window.itSyncDrive=itSaveToDrive;
window.itLoadDrive=itLoadFromDrive;

/* ── Bouton sync dans le modal IT ── */
function addSyncBtn(){
  var hdr=document.querySelector("#itModal .no-print div div");
  if(!hdr||document.getElementById("it-sync-btn"))return;
  var btn=document.createElement("button");
  btn.id="it-sync-btn";
  btn.className="btn btn-secondary";
  btn.style.cssText="border-color:rgba(34,211,238,.3);color:#22D3EE;font-size:12px;padding:7px 14px";
  btn.innerHTML="\u21bb Sync Drive";
  btn.onclick=async function(){
    btn.textContent="Sync\u2026";
    btn.disabled=true;
    await itLoadFromDrive();
    await itSaveToDrive();
    btn.innerHTML="\u21bb Sync Drive";
    btn.disabled=false;
    if(typeof toast==="function")toast("Courriers IT synchronisés","s");
  };
  hdr.insertBefore(btn,hdr.querySelector("button"));
}

/* Observer l'ouverture du modal IT */
var observer=new MutationObserver(function(){
  if(document.getElementById("itModal")&&document.getElementById("itModal").classList.contains("open")){
    addSyncBtn();
  }
});
observer.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:["class"]});

console.info("[fp_patch] IT Drive sync prêt");
