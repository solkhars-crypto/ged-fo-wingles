...
/* Retry toutes les 300ms jusqu'à 10 secondes */
var _injectTries=0;
var _injectTimer=setInterval(function(){
  _injectTries++;
  injectBtns();
  if(document.getElementById("btn-fp")||_injectTries>33)clearInterval(_injectTimer);
},300);
...
var IT_CATS={
  ...
  'egalite':{l:"\u00c9galit\u00e9 Pro",c:"#A855F7"},
...
function itRender(){
  ...
  if(sb){
    var t=itData.length;
    var a=itData.filter(function(c){return c.statut==="envoye";}).length;
    var r=itData.filter(function(c){return c.statut==="repondu";}).length;
    var e=itData.filter(function(c){return c.statut==="relance";}).length;
    sb.innerHTML=[
      {v:t,l:"Total",c:"#22D3EE"},
      {v:a,l:"En attente",c:"#3B82F6"},
      {v:r,l:"R\u00e9pondus",c:"#22C55E"},
      {v:e,l:"Relanc\u00e9s",c:"#EAB308"}
    ].map(function(x){
      return "<div class=\"it-stat\" style=\"--its-c:"+x.c+"\"><div class=\"it-stat-val\" style=\"color:"+x.c+"\">"+x.v+"</div><div class=\"it-stat-lbl\">"+x.l+"</div></div>";
    }).join("");
  }
  var el=document.getElementById("it-list");
  if(!el)return;
  if(!list.length){
    el.innerHTML="<div class=\"it-empty\">Aucun courrier \u2014 cliquez + Nouveau courrier</div>";
    return;
  }
  el.innerHTML=list.map(function(c){
    ...
    return "<div class=\"it-card\" style=\"--itc:"+cat.c+"\" onclick=\"itView('"+c.id+"')\">"
      ...
      +(c.fileData?"<span class=\"it-has-file\">"+fico+" "+c.fileName+"</span>":"")
...
function itSave(){
  ...
  var obj={
    ...
    fileData:itFileTemp?itFileTemp.data:(ex?ex.fileData:null),
...
function itEdit(id){
  ...
  if(c.fileData){
    ...
    itFileTemp={data:c.fileData,name:c.fileName,ext:ext};
    var z=document.getElementById("it-file-zone");if(z)z.classList.add("has");
    var n=document.getElementById("it-file-name");if(n)n.textContent=c.fileName||"Fichier joint";
    var s=document.getElementById("it-file-sub");if(s)s.textContent="Fichier existant";
  }else{itClearFile();}
  document.getElementById("itAddModal").classList.add("open");
}
function itDel(id){
...
function itView(id){
  ...
  if(c.fileData){dlBtn.style.display="";dlBtn.onclick=function(){var a=document.createElement("a");a.href=c.fileData;a.download=c.fileName||c.objet;a.click();};}
  else{dlBtn.style.display="none";}
  var body=document.getElementById("it-view-body");
  body.innerHTML="";body.style.textAlign="";
  if(!c.fileData){
    body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\"><div style=\"font-size:48px;margin-bottom:14px\">\u{1F4ED}</div><p>Aucun fichier joint.</p></div>";
  }else{
    var ext=(c.fileName||"").split(".").pop().toLowerCase();
    var mime=c.fileData.split(";")[0].split(":")[1]||"";
    if(mime==="application/pdf"||ext==="pdf"){
      /* Rendu PDF via PDF.js — toutes pages scrollables */
      body.style.cssText="flex:1;overflow-y:auto;overflow-x:auto;display:flex;flex-direction:column;align-items:center;padding:16px;gap:12px;background:#555";
      itRenderPDF(c.fileData, body);
    }else if(mime.startsWith("image/")||["jpg","jpeg","png","gif"].indexOf(ext)>-1){
      body.style.textAlign="center";body.style.padding="20px";
      var img=document.createElement("img");
      img.src=c.fileData;img.style.cssText="max-width:100%;border-radius:8px";
      body.appendChild(img);
    }else if(ext==="docx"||ext==="doc"){
      itRenderDocx(c.fileData,body);
    }else{
      body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\"><div style=\"font-size:48px;margin-bottom:14px\">\u{1F4C4}</div><p style=\"color:var(--text);font-weight:600\">"+(c.fileName||"Fichier")+"</p><p style=\"margin-top:8px\">T\u00e9l\u00e9chargez pour ouvrir.</p></div>";
    }
  }
  document.getElementById("itViewModal").classList.add("open");
}
function itRenderDocx(dataUrl,body){
  body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\"><div style=\"font-size:32px;margin-bottom:12px\">\u23F3</div>Conversion Word...</div>";
  function convert(){
    try{
      ...
      mammoth.convertToHtml({arrayBuffer:arr.buffer},{
        ...
        }else{body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\">Document vide.</div>";}
      }).catch(function(){body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\">Erreur conversion.</div>";});
    }catch(e){body.innerHTML="<div style=\"text-align:center;padding:48px;color:var(--muted)\">Erreur lecture.</div>";}
  }
  if(typeof mammoth!=="undefined"){convert();}
  else{
    var s=document.createElement("script");
    s.src="https://cdnjs.cloudflare.com/ajax/libs/mammoth/1.6.0/mammoth.browser.min.js";
    s.onload=convert;document.head.appendChild(s);
  }
}

function itRenderPDF(dataUrl, body){
  body.innerHTML='<div style="text-align:center;padding:48px;color:#fff"><div style="font-size:32px;margin-bottom:12px">\u23F3</div>Chargement PDF...</div>';
  function doRender(){
    try{
      ...
      pdfjsLib.getDocument({data:arr}).promise.then(function(pdf){
        ...
        nav.style.cssText="position:sticky;top:0;z-index:10;background:rgba(0,0,0,.75);color:#fff;padding:8px 16px;border-radius:8px;display:flex;align-items:center;gap:12px;font-size:13px;margin-bottom:8px;align-self:stretch;justify-content:center";
        nav.innerHTML='<button id="it-pdf-prev" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:5px 12px;border-radius:5px;cursor:pointer;font-size:14px">&#8249;</button>'
          +'<span id="it-pdf-label" style="min-width:80px;text-align:center">Page 1 / '+total+'</span>'
          +'<button id="it-pdf-next" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:5px 12px;border-radius:5px;cursor:pointer;font-size:14px">&#8250;</button>'
          +'<button id="it-pdf-zm" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:5px 10px;border-radius:5px;cursor:pointer;font-size:12px">+</button>'
          +'<button id="it-pdf-zp" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:5px 10px;border-radius:5px;cursor:pointer;font-size:12px">\u2212</button>';
        body.appendChild(nav);
        var scale=1.4;
        var canvases=[];
        function renderAll(){
          /* Supprimer anciens canvases */
          ...
          for(var p=1;p<=total;p++){
            (function(pnum){
              pdf.getPage(pnum).then(function(page){
                ...
                wrap.style.cssText="box-shadow:0 4px 20px rgba(0,0,0,.5);border-radius:4px;overflow:hidden;flex-shrink:0";
                ...
                lbl.style.cssText="font-size:10px;color:rgba(255,255,255,.4);margin-bottom:8px;text-align:center";
                lbl.textContent="Page "+pnum+" / "+total;
        ...
        renderAll();
        /* Zoom */
        document.getElementById("it-pdf-zm").onclick=function(){scale=Math.min(scale+0.2,3);renderAll();};
        document.getElementById("it-pdf-zp").onclick=function(){scale=Math.max(scale-0.2,0.5);renderAll();};
        /* Navigation prev/next */
        var curPage=1;
        function updateNav(){document.getElementById("it-pdf-label").textContent="Page "+curPage+" / "+total;}
        document.getElementById("it-pdf-prev").onclick=function(){
          if(curPage>1){curPage--;updateNav();
            var items=body.querySelectorAll("div[style*=box-shadow]");
            if(items[curPage-1])items[curPage-1].scrollIntoView({behavior:"smooth"});}};
        document.getElementById("it-pdf-next").onclick=function(){
          if(curPage<total){curPage++;updateNav();
            var items=body.querySelectorAll("div[style*=box-shadow]");
            if(items[curPage-1])items[curPage-1].scrollIntoView({behavior:"smooth"});}};
      }).catch(function(e){
        console.warn("[PDF.js]",e);
        itPDFFallback(dataUrl,body);
      });
    }catch(e){
  ...
  if(typeof pdfjsLib!=="undefined"){doRender();}
  else{
    /* PDF.js pas encore chargé - charger la lib */
    var s=document.createElement("script");
    s.src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js";
    s.onload=doRender;
    s.onerror=function(){itPDFFallback(dataUrl,body);};
    document.head.appendChild(s);
  }
}
function itPDFFallback(dataUrl,body){
  body.style.cssText="flex:1;overflow-y:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px";
  body.innerHTML='<div style="text-align:center;color:#fff"><div style="font-size:48px;margin-bottom:14px">\u{1F4C4}</div>'
    +'<p style="font-size:14px;font-weight:600;margin-bottom:12px">PDF</p>'
    +'<p style="font-size:12px;opacity:.6;margin-bottom:16px">Pr\u00e9visualisation non disponible sur cet appareil.</p>'
    +'<a href="'+dataUrl+'" download="document.pdf" style="text-decoration:none">'
    +'<button style="background:linear-gradient(135deg,#E03A47,#B91C2C);color:#fff;border:none;padding:10px 22px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600">\u2193 T\u00e9l\u00e9charger le PDF</button>'
    +'</a></div>';
}

function itOpen(){
  itLoad();
  /* Charger depuis Drive si connecté */
...
/* DRIVE SYNC — fichier dédié GED_FO_IT_data.json */
(function(){
  ...
  async function findOrCreate(){
    ...
    if(!_fid){_fid=localStorage.getItem(FID_KEY)||null;}
    if(_fid)return _fid;
    try{
      ...
      if(d.files&&d.files[0]){
        _fid=d.files[0].id;localStorage.setItem(FID_KEY,_fid);return _fid;
      }
    }catch(e){}
    try{
  ...
  async function saveToCloud(){
    ...
    try{
      ...
      var payload=JSON.stringify({itCourriers:itData,saved:new Date().toISOString()});
      var r=await fetch("https://www.googleapis.com/upload/drive/v3/files/"+fid+"?uploadType=media",
        {method:"PATCH",headers:{"Content-Type":"application/json","Authorization":"Bearer "+tok},body:payload});
      if(r.ok)console.info("[IT Drive] Sauvé",itData.length,"courriers");
      else console.warn("[IT Drive save] HTTP",r.status);
  ...
  async function loadFromCloud(showToast){
    if(!connected()){
      if(showToast&&typeof toast==="function")toast("Connectez-vous à Drive pour synchroniser","i");
      return false;
    }
    try{
      ...
      var r=await fetch("https://www.googleapis.com/drive/v3/files/"+fid+"?alt=media",
        {headers:{Authorization:"Bearer "+tok}});
      if(!r.ok){console.warn("[IT Drive load] HTTP",r.status);return false;}
      var d=await r.json();
      ...
      console.info("[IT Drive] Charg\u00e9",merged.length,"courriers");
      return true;
    }catch(e){console.warn("[IT Drive load]",e);return false;}
  }

  /* Hook appelé par itStore() */
  window._itSyncNow=function(){saveToCloud();};

  /* Hook appelé par itOpen() — charge toujours depuis Drive */
  window._itLoadDrive=function(){
  ...
  window.itManualSync=async function(){
    ...
    var ok=await loadFromCloud(false);
    if(ok)await saveToCloud();
    if(btn){
    ...
    if(typeof toast==="function")toast(ok?"Courriers IT synchronis\u00e9s \u2713":"Connectez-vous d'abord \u00e0 Drive","s");
  };

  /* Surveiller en continu l'apparition du token Drive
     (pas de limite de temps — fonctionne même si l'user se connecte tard) */
  setInterval(function(){
    var tok=getToken();
    if(tok&&tok!==_lastTok){
      /* Token vient d'apparaître ou de changer → charger */
      _lastTok=tok;
      _fid=localStorage.getItem(FID_KEY)||null;
      loadFromCloud(false);
      console.info("[IT Drive] Token détecté, chargement...");
    }
  },1000);

  console.info("[IT Drive] Sync v2 prêt");
})();

/* ─ 8. PATCH VISUALISEUR WORD GED ──────────────────────────── */

/* PATCH VISUALISEUR GED — Word via mammoth */
(function(){
  if(!document.getElementById("docx-css")){
    var s=document.createElement("style");s.id="docx-css";
  ...
  window._vOpenOffice=function(driveId,fileName,fileData){
    ...
    if(ext==="docx"||ext==="doc"){
      if(!fileData){
        body.innerHTML="<div class=\"no-preview\"><div class=\"np-ico\">\u{1F4DD}</div><p style=\"color:var(--text);font-size:14px;font-weight:600\">"+fileName+"</p>"+(driveId?"<a href=\"https://drive.google.com/file/d/"+driveId+"/view\" target=\"_blank\" style=\"text-decoration:none;margin-top:12px;display:inline-block\"><button class=\"btn btn-primary\">\u2197 Ouvrir dans Drive</button></a>":"<p style=\"margin-top:8px\">Aucun fichier disponible.</p>")+"</div>";
      ...
      if(driveId){
        ...
        body.innerHTML="<div class=\"no-preview\"><div class=\"np-ico\">"+ico+"</div><p style=\"color:var(--text);font-weight:600\">"+fileName+"</p><p style=\"margin-top:8px\">Ouvert dans Google Drive.</p><a href=\"https://drive.google.com/file/d/"+driveId+"/view\" target=\"_blank\" style=\"text-decoration:none;margin-top:12px;display:inline-block\"><button class=\"btn btn-primary\">\u2197 Ouvrir dans Drive</button></a></div>";
      }else if(fileData){
