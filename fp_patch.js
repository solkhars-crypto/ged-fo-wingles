...
/* 9 — SYNC DRIVE POUR COURRIERS IT */
(function(){
  function patchDriveSync(){
    /* Attendre que la GED soit initialisée */
    var maxTry=20, tries=0;
    var timer=setInterval(function(){
      tries++;
      if(tries>maxTry){clearInterval(timer);return;}
      
      /* Vérifier que saveToDrive existe dans la GED */
      if(typeof saveToDrive!=='function'){return;}
      clearInterval(timer);
      
      /* ── Surcharger saveToDrive pour inclure itData ── */
      var _origSave=saveToDrive;
      window.saveToDrive=function(){
        /* Sauvegarder itData en localStorage */
        try{localStorage.setItem('fo_it',JSON.stringify(itData));}catch(e){}
        /* Appeler l'original — il lit les variables globales docs/meetings/fiches/avis */
        /* On injecte itData dans le payload en patchant JSON.stringify temporairement */
        return _origSave.apply(this, arguments);
      };

      /* ── Surcharger loadFromDrive pour récupérer itData ── */
      var _origLoad=loadFromDrive;
      window.loadFromDrive=async function(){
        var result=await _origLoad.apply(this,arguments);
        /* Après le chargement, lire itData depuis le fichier Drive si disponible */
        /* Le fichier GED_FO_Wingles_data.json contient maintenant itData */
        try{
          if(window.DATA_FILE_ID&&window.accessToken){
            var r2=await fetch('https://www.googleapis.com/drive/v3/files/'+window.DATA_FILE_ID+'?alt=media',{
              headers:{Authorization:'Bearer '+window.accessToken}
            });
            if(r2.ok){
              var d2=await r2.json();
              if(d2.itCourriers&&Array.isArray(d2.itCourriers)){
                itData=d2.itCourriers;
                localStorage.setItem('fo_it',JSON.stringify(itData));
                console.info('[fp_patch] IT courriers chargés depuis Drive:',itData.length);
              }
            }
          }
        }catch(e){console.warn('[fp_patch] Lecture IT Drive:',e);}
        return result;
      };

      /* ── Patcher aussi la sauvegarde Drive pour inclure itCourriers ── */
      /* On patche JSON.stringify sur l'objet payload uniquement */
      var _origSave2=saveToDrive;
      window.saveToDrive=async function(){
        /* Sauver localement */
        try{localStorage.setItem('fo_it',JSON.stringify(itData));}catch(e){}
        
        if(!window.driveConnected||!window.accessToken){
          localStorage.setItem('fo_docs',JSON.stringify(window.docs||[]));
          localStorage.setItem('fo_meetings',JSON.stringify(window.meetings||[]));
          localStorage.setItem('fo_fiches',JSON.stringify(window.fiches||[]));
          localStorage.setItem('fo_avis',JSON.stringify(window.avis||[]));
          return;
        }
        try{
          var payload=JSON.stringify({
            ...
            fiches:window.fiches||[],
            avis:window.avis||[],
            itCourriers:itData||[],
            lastSaved:new Date().toISOString()
          });
          var blob=new Blob([payload],{type:'application/json'});
          if(window.DATA_FILE_ID){
            await fetch('https://www.googleapis.com/upload/drive/v3/files/'+window.DATA_FILE_ID+'?uploadType=media',{
              method:'PATCH',
              headers:{'Content-Type':'application/json','Authorization':'Bearer '+window.accessToken},
              body:blob
            });
          }else{
            var meta={name:'GED_FO_Wingles_data.json',description:'GED Force Ouvriere O-I Wingles'};
            ...
            var rd=await r.json();
            window.DATA_FILE_ID=rd.id;
            localStorage.setItem('fo_data_fid',rd.id);
          }
          ...
          console.error('[fp_patch] Erreur saveToDrive IT:',e);
        }
      };

      /* ── Surcharger itStore pour aussi appeler saveToDrive ── */
      var _origItStore=window.itStore;
      window.itStore=function(){
        localStorage.setItem('fo_it',JSON.stringify(itData));
        /* Synchroniser sur Drive si connecté */
        if(typeof saveToDrive==='function'&&window.driveConnected){
          saveToDrive();
        }
      };

      /* ── Charger itData depuis Drive au démarrage si déjà connecté ── */
      if(window.driveConnected&&window.DATA_FILE_ID&&window.accessToken){
        fetch('https://www.googleapis.com/drive/v3/files/'+window.DATA_FILE_ID+'?alt=media',{
          headers:{Authorization:'Bearer '+window.accessToken}
        }).then(function(r){return r.json();}).then(function(d){
          if(d.itCourriers&&Array.isArray(d.itCourriers)){
            itData=d.itCourriers;
            localStorage.setItem('fo_it',JSON.stringify(itData));
            if(document.getElementById('it-list'))itRender();
            console.info('[fp_patch] IT courriers sync Drive:',itData.length);
          }
        }).catch(function(e){console.warn('[fp_patch] IT Drive init:',e);});
      }

      /* Aussi écouter l'événement de connexion Drive */
      var _origOnDriveConnected=window.onDriveConnected;
      if(typeof _origOnDriveConnected==='function'){
        window.onDriveConnected=async function(){
          await _origOnDriveConnected.apply(this,arguments);
          /* Après connexion, charger les courriers IT */
          if(window.DATA_FILE_ID&&window.accessToken){
            try{
              var r=await fetch('https://www.googleapis.com/drive/v3/files/'+window.DATA_FILE_ID+'?alt=media',{
                headers:{Authorization:'Bearer '+window.accessToken}
              });
              if(r.ok){
                var d=await r.json();
            ...
            }catch(e){console.warn('[fp_patch] IT onDriveConnected:',e);}
          }
        };
      }

      console.info('[fp_patch] Sync Drive IT courriers activé');
    },300);
  }

  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',patchDriveSync);
  }else{
    patchDriveSync();
  }
})();
