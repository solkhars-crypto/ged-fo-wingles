...
(function(){
  ...
  function getToken(){
    /* Les variables de la GED sont dans un IIFE, pas sur window.
       On lit depuis localStorage où la GED sauvegarde le token. */
  ...
  function connected(){
    var tok=getToken();
    if(!tok)return false;
    /* driveConnected peut être sur window ou dans l'IIFE */
    var dc=window.driveConnected||localStorage.getItem("fo_tok")!==null;
    return !!(dc&&tok);
  }
  ...
  async function findOrCreate(){
    ...
    }catch(e){console.warn("[IT]créer fichier:",e);}
    return null;
  }
  ...
  async function saveToCloud(){
    ...
    try{
      ...
      await fetch("https://www.googleapis.com/upload/drive/v3/files/"+fid+"?uploadType=media",
        {method:"PATCH",headers:{"Content-Type":"application/json","Authorization":"Bearer "+getToken()},body:payload});
      console.info("[IT Drive] Sauvé",itData.length,"courriers");
  ...
  async function loadFromCloud(showToast){
    if(!connected())return;
    try{
      var fid=await findOrCreate();if(!fid)return;
      var r=await fetch("https://www.googleapis.com/drive/v3/files/"+fid+"?alt=media",
        {headers:{Authorization:"Bearer "+getToken()}});
      if(!r.ok)return;
      var d=await r.json();
      if(!d.itCourriers||!Array.isArray(d.itCourriers))return;
      /* Merger : prendre tous les IDs des deux sources */
      var map={};
      ...
      if(showToast&&merged.length&&typeof toast==="function")
        toast(merged.length+" courriers IT charg\u00e9s","s");
      console.info("[IT Drive] Charg\u00e9",merged.length,"courriers");
    }catch(e){console.warn("[IT Drive load]",e);}
  }

  /* Hook appelé par itStore() à chaque modification */
  window._itSyncNow=function(){saveToCloud();};

  /* Hook appelé par itOpen() */
  window._itLoadDrive=function(){
    loadFromCloud(true).then(function(){itRender();});
  };
  ...
  /* Auto-chargement : surveiller la connexion Drive */
  var _tries=0;
  var _poll=setInterval(function(){
    _tries++;
    if(_tries>240){clearInterval(_poll);return;}
    if(connected()){clearInterval(_poll);loadFromCloud(false);}
  },500);

  console.info("[IT Drive] Sync prêt");
})();
