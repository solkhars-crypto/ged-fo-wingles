...
function itRenderPDF(dataUrl, body){
  body.style.cssText="flex:1;display:flex;align-items:center;justify-content:center;padding:24px;background:#1a1a2e";
  try{
    ...
    /* Détecter iOS/Safari tablette */
    var isIOS=/iPad|iPhone|iPod/.test(navigator.userAgent)||
      (navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1);
    var isSafari=/^((?!chrome|android).)*safari/i.test(navigator.userAgent);

    if(isIOS||isSafari){
      /* Sur iOS/Safari : ouvrir dans nouvel onglet */
      body.innerHTML='<div style="text-align:center;color:#fff;max-width:320px">'
        +'<div style="font-size:64px;margin-bottom:20px">\u{1F4C4}</div>'
        +'<p style="font-size:16px;font-weight:700;margin-bottom:8px">PDF</p>'
        +'<p style="font-size:13px;opacity:.7;line-height:1.6;margin-bottom:24px">'
        +'Safari ne peut pas afficher les PDF dans cette fen\u00eatre.<br>'
        +'Cliquez ci-dessous pour l\u2019ouvrir.</p>'
        +'<button id="it-pdf-open" style="background:linear-gradient(135deg,#E03A47,#B91C2C);color:#fff;border:none;padding:14px 28px;border-radius:10px;cursor:pointer;font-size:15px;font-weight:700;width:100%">'
        +'\u{1F4C4} Ouvrir le PDF</button>'
        +'<p style="font-size:11px;opacity:.4;margin-top:12px">S\u2019ouvrira dans un nouvel onglet</p>'
        +'</div>';
      document.getElementById("it-pdf-open").onclick=function(){
        window.open(url,"_blank");
      };
    }else{
      /* PC/Android Chrome : iframe classique */
      var ifr=document.createElement("iframe");
      ifr.src=url;
      ifr.style.cssText="width:100%;flex:1;height:100%;border:none;background:#fff";
      body.style.cssText="flex:1;overflow:hidden;display:flex;flex-direction:column;padding:0";
      body.innerHTML="";
      body.appendChild(ifr);
    }
    /* Nettoyer blob URL à la fermeture */
    var modal=document.getElementById("itViewModal");
    if(modal){
      var once=function(){URL.revokeObjectURL(url);modal.removeEventListener("click",once);};
      modal.addEventListener("click",once);
    }
  }catch(e){
    console.warn("[PDF]",e);
...
function itPDFFallback(dataUrl,body){
  body.style.cssText="flex:1;display:flex;align-items:center;justify-content:center;padding:24px";
  body.innerHTML='<div style="text-align:center;color:#fff">'
    +'<div style="font-size:48px;margin-bottom:14px">\u{1F4C4}</div>'
    +'<p style="font-size:13px;opacity:.7;margin-bottom:16px">Impossible d\u2019afficher ce PDF ici.</p>'
    +'<a href="'+dataUrl+'" download="document.pdf" style="text-decoration:none">'
    +'<button style="background:linear-gradient(135deg,#E03A47,#B91C2C);color:#fff;border:none;padding:10px 22px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600">\u2193 T\u00e9l\u00e9charger</button>'
    +'</a></div>';
}
