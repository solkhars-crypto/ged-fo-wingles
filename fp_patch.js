...
function itRenderPDF(dataUrl, body){
  body.style.cssText="flex:1;overflow:hidden;display:flex;flex-direction:column;padding:0;background:#555";
  try{
    /* Convertir base64 en blob URL — le navigateur affiche toutes les pages */
    ...
    /* Libérer le blob URL quand le viewer se ferme */
    var modal=document.getElementById("itViewModal");
    if(modal){
      var _cleanup=function(){URL.revokeObjectURL(url);modal.removeEventListener("click",_cleanup);};
      modal.addEventListener("click",_cleanup);
    }
    body.innerHTML="";
    body.appendChild(ifr);
  }catch(e){
    console.warn("[PDF blob]",e);
