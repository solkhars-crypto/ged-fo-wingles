...
/* 9 — SYNC DRIVE COURRIERS IT (version simple et fiable) */
(function(){

  /* Attendre que la GED soit prête */
  function waitForGED(cb){
    var n=0;
    var t=setInterval(function(){
      n++;
      if(n>40){clearInterval(t);return;}
      if(typeof saveToDrive==="function"&&typeof loadFromDrive==="function"){
        clearInterval(t);
        cb();
      }
    },250);
  ...
  waitForGED(function(){
    ...
    /* ── 1. Surcharger saveToDrive pour inclure itCourriers ── */
    var _origSave=window.saveToDrive;
    window.saveToDrive=async function(){
      ...
      if(!window.driveConnected||!window.accessToken){
        /* Pas connecté : sauvegarder localement uniquement */
        try{
      ...
      try{
        ...
        var headers={"Content-Type":"application/json","Authorization":"Bearer "+window.accessToken};

        if(window.DATA_FILE_ID){
          await fetch("https://www.googleapis.com/upload/drive/v3/files/"+window.DATA_FILE_ID+"?uploadType=media",{
            method:"PATCH",headers:headers,body:blob
          });
        }else{
        ...
        console.error("[fp_patch] saveToDrive IT error:",e);
      }
    };

    /* ── 2. Surcharger loadFromDrive pour lire itCourriers ── */
    var _origLoad=window.loadFromDrive;
    window.loadFromDrive=async function(){
      /* Appeler l'original pour charger docs/meetings/fiches/avis */
      await _origLoad.apply(this,arguments);

      /* Relire le fichier pour récupérer itCourriers */
      if(!window.DATA_FILE_ID||!window.accessToken)return;
      try{
        ...
        if(!r.ok)return;
        var d=await r.json();
        if(d.itCourriers&&Array.isArray(d.itCourriers)&&d.itCourriers.length>0){
          itData=d.itCourriers;
          localStorage.setItem("fo_it",JSON.stringify(itData));
          if(document.getElementById("it-list"))itRender();
          if(typeof toast==="function")toast("Courriers IT synchronisés ("+itData.length+")","s");
          console.info("[fp_patch] IT courriers chargés depuis Drive:",itData.length);
        }
      }catch(e){
        console.warn("[fp_patch] loadFromDrive IT:",e);
      }
    };

    /* ── 3. Surcharger onDriveConnected pour charger itCourriers ── */
    var _origODC=window.onDriveConnected;
    if(typeof _origODC==="function"){
      window.onDriveConnected=async function(){
        await _origODC.apply(this,arguments);
        /* Après connexion, charger les courriers IT depuis Drive */
        if(!window.DATA_FILE_ID||!window.accessToken)return;
        try{
          var r=await fetch("https://www.googleapis.com/drive/v3/files/"+window.DATA_FILE_ID+"?alt=media",{
            headers:{Authorization:"Bearer "+window.accessToken}
          ...
          if(d.itCourriers&&Array.isArray(d.itCourriers)&&d.itCourriers.length>0){
            ...
            localStorage.setItem("fo_it",JSON.stringify(itData));
            if(document.getElementById("it-list"))itRender();
            console.info("[fp_patch] IT courriers chargés après connexion:",itData.length);
          }
        }catch(e){
          console.warn("[fp_patch] onDriveConnected IT:",e);
        }
      };
    }
