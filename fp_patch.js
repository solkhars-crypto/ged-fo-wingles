...
function itViewFile(id){
  /* Utiliser LE VISUALISEUR DE LA GED */
  var c=itData.find(function(x){return x.id===id;});if(!c)return;
  var viewerOv=document.getElementById("viewerOv");
  if(!viewerOv){
    if(typeof toast==="function")toast("Visualiseur non disponible","e");
    return;
  }
  ...
  if(dlBtn){
    dlBtn.onclick=function(){
      if(c.driveId){window.open("https://drive.google.com/file/d/"+c.driveId+"/view","_blank");}
    };
  }
  /* Dispatcher vers le bon renderer */
  if(typeof _vDispatch==="function"){
    _vDispatch(null,c.fileName||c.objet,c.driveId||null);
  }else if(c.driveId){
    var ext=(c.fileName||"").split(".").pop().toLowerCase();
    if(typeof _vOpenOffice==="function"||ext==="pdf"){
      if(ext==="pdf"&&typeof _vOpenPdf==="function"){
        _vOpenPdf("https://drive.google.com/file/d/"+c.driveId+"/view",true);
      }else if(typeof _vOpenOffice==="function"){
        _vOpenOffice(c.driveId,c.fileName,null);
      }
    }
  }
  /* Fermer modal IT et ouvrir viewer GED */
